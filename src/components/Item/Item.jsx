    ...
    <>
      ...
      <div className='item_right'>
        <div className='item_top'>
          <h5 title={title.length >= 32 ? title : null}>
            {truncate(title, 60)}
          </h5>
        ...
        <ul className='item_genres'>
          {setGenres(genresList, genres).map((genre) => (
            <li key={genre} className='item_genre'>
              {genre}
            </li>
          ))}
        </ul>

        <div className='item_text'>{truncate(overview, 105)}</div>
